...
const localText = {
  title: { en: 'Oven Services', ar: 'أقسام' },
  titleItalic: { en: 'Categories', ar: 'الأفران' },
...
export default function OvenCategories() {
  const { lang, dir } = useLang();
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 6; 

  const totalPages = Math.ceil(ovenData.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = ovenData.slice(startIndex, startIndex + itemsPerPage);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentItems.map((item) => (
            <Link href={`/services/oven/${item.id}`} key={item.id} className="block group">
