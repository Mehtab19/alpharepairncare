...
const localText = {
  title: { en: 'Repair', ar: 'أقسام' },
  titleItalic: { en: 'Categories', ar: 'الصيانة' },
...
export default function FridgeCategories() {
  const { lang, dir } = useLang();
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 6;

  const totalPages = Math.ceil(fridgeData.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = fridgeData.slice(startIndex, startIndex + itemsPerPage);
