...
const localText = {
  title: { en: 'AC Services', ar: 'أقسام' },
  titleItalic: { en: 'Categories', ar: 'التكييف' },
...
export default function AcCategories() {
  const { lang, dir } = useLang();
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 3; // Forces pagination for 4 items!

  const totalPages = Math.ceil(acData.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = acData.slice(startIndex, startIndex + itemsPerPage);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentItems.map((item) => (
            <Link href={`/services/ac/${item.id}`} key={item.id} className="block group">
