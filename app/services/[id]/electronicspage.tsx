...
export default function ElectronicsDetailsPage({ params }: { params: { id: string } }) {
  const { lang, dir } = useLang();
  
  const currentService = electronicsData.find((s) => s.id === params.id);
  if (!currentService) return notFound();

  const sidebarServices = electronicsData.filter(s => s.id !== params.id).slice(0, 5); 
    ...
    <div className="min-h-screen bg-ink text-white pt-32 pb-24 px-6 md:px-10" dir={dir}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className={cn("text-xs tracking-wider uppercase text-white/40 mb-12", lang === 'ar' && 'font-arabic')}>
          ...
          <Link href="/services/electronics" className="hover:text-gold transition-colors">
            {lang === 'en' ? 'Electronics Maintenance' : 'صيانة الإلكترونيات'}
          </Link>
          <span className="mx-2">/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
          ...
          {/* Sidebar only shows if there are other services */}
          {sidebarServices.length > 0 && (
            <aside className="lg:col-span-3 order-2 lg:order-1 border-t lg:border-t-0 lg:border-r border-white/10 pt-10 lg:pt-0 lg:pr-10 rtl:lg:border-r-0 rtl:lg:border-l rtl:lg:pl-10">
              <h3 className={cn("text-lg font-display text-white mb-8 border-b border-white/10 pb-4", lang === 'ar' && 'font-arabic')}>
                {lang === 'en' ? 'Other Electronics Services' : 'خدمات الإلكترونيات الأخرى'}
              </h3>
              <div className="space-y-6">
                {sidebarServices.map((item) => (
                  <Link href={`/services/electronics/${item.id}`} key={item.id} className="flex gap-4 group">
          ...
          <main className={cn("order-1 lg:order-2", sidebarServices.length > 0 ? "lg:col-span-9" : "lg:col-span-12")}>
